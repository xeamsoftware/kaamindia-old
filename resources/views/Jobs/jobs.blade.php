...
<div class="page-content-area">
	...
	<section class="section-padding dashboard-section">
		<div class="container">
			...
			<div class="d-lg-none">
				<div class="dash-data-job">
					<div class="dash-data-text">
						<h2>Filter</h2>
					</div>
					<div class="dash-data-img">
						<img src="{{asset('assets')}}/images/filter.svg" alt="">
					</div>
				</div>
			</div>
			<div class="row justify-content-center">
				<div class=" col-lg-4 mb-box dash-data-box ">
								...
								<div class="form-group mb-0 ">
									<div class="custom-control custom-checkbox mb-checkbox">
										<input
											...
											<?= (isset($parse_in_url['job_type']) && in_array('Permanent',$parse_in_url['job_type'])) ? 'checked' : '' ;?>
										/>
									...
									<div class="custom-control custom-checkbox mb-checkbox">
										<input
											...
											<?= (isset($parse_in_url['job_type']) && in_array('Contractual',$parse_in_url['job_type'])) ? 'checked' : '' ;?>
										/>
					...
					<div class="row">
						@if(isset($filter_job))
							...
							@foreach($filter_job as $val)
								<div class="job-box col">
									<div class="job-like-toggle">
										<?php $checked = (is_array($saved_by_current_user) && in_array($val->id,$saved_by_current_user)) ? "checked" : "";  ?>
										<input type="checkbox" class="job-like-input" data-id="{{$val->id}}" id="joblike1{{$val->id}}" data-login-url="{{ url('login-as-employer-or-jobseeker') }}" data-userid="{{ !empty($user_id) ? $user_id : '' }}" <?= $checked; ?>/>
									...
									<a href="{{url('/jobs-detail')}}/{{$val->job_slug}}">
										<div class="job-left-box">
											...
											<div class="job-main-data job-main-box">
												<div>
												<h5>{{$val->job_title}}</h5>
												<ul>
						...
						@else
							@foreach($Job as $val)
								<div class="job-box col">
									<div class="job-like-toggle">
										<?php $checked = (is_array($saved_by_current_user) && in_array($val->id,$saved_by_current_user)) ? "checked" : "";  ?>
										 <input type="checkbox" class="job-like-input" data-id="{{$val->id}}" data-userid="{{ !empty($user_id) ? $user_id : '' }}" data-login-url="{{ url('login-as-employer-or-jobseeker') }}" id="joblike1{{$val->id}}" <?= $checked; ?> />
										<label for="joblike1{{$val->id}}" class="job-like-label">
											<i class="far fa-heart"></i>
										</label>
									</div>
								...
								<ul class="pagination">
									@if(($filter_job_count) > 1)
										...
										@endif
											...
											@else
												<li class="page-item page-prvnxt-btn">
